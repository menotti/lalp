...
#include "if_ge_op_s.h"
#include "string"
#include "../Aux/FuncoesAux.h"

using namespace std;

if_ge_op_s::if_ge_op_s(void*node, int dataWidth) : Componente(node) {
    this->setNomeCompVHDL("if_ge_op_s");
...
if_ge_op_s::~if_ge_op_s() {
}

void if_ge_op_s::createAllPorts(){
...
void if_ge_op_s::createAllGeneric(){
...
string if_ge_op_s::getEstruturaComponenteVHDL(){
    string res = "";
...
void if_ge_op_s::setValor(const string &val){
    this->valor = val;
    this->setGenericMapVal("initial", "VAL", val);
}

string if_ge_op_s::getTipo(){
    return this->tipo_var;
}

string if_ge_op_s::getCondEsq(){
    return(this->I0);
}

string if_ge_op_s::getCondDir(){
    return(this->I1);
}

void if_ge_op_s::setCondEsq(const string &aux){
    this->I0 = aux;
}

void if_ge_op_s::setCondDir(const string &aux){
    this->I1 = aux;
}

void if_ge_op_s::setTipo(const string &aux){
    this->tipo_var = aux;
}
