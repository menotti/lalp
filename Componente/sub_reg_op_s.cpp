...
#include "sub_reg_op_s.h"
#include "string"
#include "../Aux/FuncoesAux.h"

using namespace std;

sub_reg_op_s::sub_reg_op_s(void*node, const string &aux, int dataWidth) : Componente(node, aux) {
    this->dataWidth = dataWidth;
    this->setNomeCompVHDL("sub_reg_op_s");    
    this->tipo_comp = CompType::REG;    
    this->createAllPorts();
    this->createAllGeneric();
}

sub_reg_op_s::~sub_reg_op_s() {
}

void sub_reg_op_s::createAllPorts(){
...
void sub_reg_op_s::createAllGeneric(){
...
string sub_reg_op_s::getEstruturaComponenteVHDL(){
    string res = "";
...
string sub_reg_op_s::geraDOTComp(){
    string dataWidthAux    = FuncoesAux::IntToStr(this->dataWidth);
    string res = "";
    res += "\""+this->getName()+"\" [shape=record, fontcolor=blue, style=\"filled\", fillcolor=\"lightgray\", label=\"{{<I0>I0["+dataWidthAux+"]|<I1>I1["+dataWidthAux+"]|<Sel1>Sel1[1]|<clk>clk|<reset>reset|<we>we}|sub_reg_op_s:"+this->getName()+"|{<O0>O0["+dataWidthAux+"]}}\"]; \n";
    return res;
}

void sub_reg_op_s::setValor(const string &val){
    this->valor = val;
    this->setGenericMapVal("initial", "VAL", val);
}

string sub_reg_op_s::getTipo(){
    return this->tipo_var;
}

void sub_reg_op_s::setTipo(const string &aux){
    this->tipo_var = aux;
}
