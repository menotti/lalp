 ...
 */
public class add_reg_op_s extends IComponent {
	private long initialValue = 0;
	public add_reg_op_s() {
		this(Parameters.getDefaultDataWidth());
	}
	public add_reg_op_s(int width) {
		super(width);
		Generic generic;
	...
	public add_reg_op_s(long width) {
		this((int)width);
	}
	public add_reg_op_s(String name) {
		this();
		super.name = name;
	}
	public add_reg_op_s(String name, int width) {
		this(width);
		super.name = name;
	}
	public add_reg_op_s(String name, long width) {
		this(width);
		super.name = name;
	}
	public String getVHDLDeclaration() {
		String d = new String();
		d += "component add_reg_op_s\n";  
		d += "generic (\n";
		d += "	w_in1	: integer := 16;\n";
		d += "	w_in2	: integer := 16;\n";
		d += "	w_out	: integer := 32;\n";
