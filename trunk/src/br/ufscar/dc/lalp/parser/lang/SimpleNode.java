...
public class SimpleNode implements Node {
	...
	private void connectMux() {
		...
		if (getConnections().getSecond().id == ALPParserTreeConstants.JJTLITERAL) {
			...
			cFrom = parser.design.addComponent(new const_op(new Integer(getConnections().getSecond().jjtGetValue().toString()), width));
		}
		else {
	...
	private Component createConstant(SimpleNode n) {
		...
		if (n.id == ALPParserTreeConstants.JJTLITERAL) {
			c = parser.design.addComponent(new const_op((Long)n.value, getWidth()));
		}
		else {
			String name = n.getIdentifier();
			Integer width = parser.allConsts.get(name);
			c = parser.design.addComponent(new const_op(name, width, getWidth()));
		}
		return c;
	}
