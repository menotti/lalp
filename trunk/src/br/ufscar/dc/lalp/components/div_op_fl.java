...
/**
 * The DIV operation for Floating-Point (32-Bits IEEE 754).
 * 
 * @author </a>
 * @author </a>
 * @version 
 */
public class div_op_fl extends IComponent {
	public div_op_fl() {
		this(Parameters.getDefaultDataWidth());
	}
	public div_op_fl(int width) {
		super(width);
		setSync(false);
		Generic generic;
	...
	public div_op_fl(String name) {
		this();
		super.name = name;
	}
	public div_op_fl(String name, int width) {
		this(width);
		super.name = name;
	}
	public String getVHDLDeclaration() {
		String d = new String();
		d += "component div_op_fl\n";  
		d += "generic (\n";
		d += "	w_in1	: integer := 32;\n";
		d += "	w_in2	: integer := 32;\n";
		d += "	w_out	: integer := 32;\n";
