 ...
 * @deprecated
 */
public class dotprod_hw extends Design {
	...
	public dotprod_hw() {
		...
		delay_op stepDelayed = (delay_op)this.addComponent(new delay_op("stepDelayed", 1, 1));
		try {
			init.connectComponent(for1, "clk_en");
			for1.connectComponent("done", doneDelayed);
			doneDelayed.connectComponent(done);
			for1.connectComponent("step", stepDelayed);
			...
			stepDelayed.connectComponent(acc1, "we");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
