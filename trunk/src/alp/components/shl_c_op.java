 ...
 */
public class shl_c_op extends IComponent {
	protected int amount = 1;
	public shl_c_op() {
		this(1);
	}
	public shl_c_op(int amount) {
		this(amount, Parameters.getDefaultDataWidth());
	}
	public shl_c_op(int amount, int width) {
		super(width);
		setSync(false);
		Generic generic;
	...
	public shl_c_op(String name) {
		this();
		super.name = name;
	}
	public shl_c_op(String name, int amount) {
		this(amount);
		super.name = name;
	}
	public shl_c_op(String name, int amount, int width) {
		this(amount, width);
		super.name = name;
	}
