...
/**
 * The signed NE (not equal) operation.
 * 
 ...
 */
public class if_ne_op_s extends IComponent {
	public if_ne_op_s() {
		this(Parameters.getDefaultDataWidth());
	}
	public if_ne_op_s(int width) {
		super(width);
		setSync(false);
		Generic generic;
	...
	public if_ne_op_s(String name) {
		this();
		super.name = name;
	}
	public if_ne_op_s(String name, int width) {
		this(width);
		super.name = name;
	}
	public String getVHDLDeclaration() {
		String d = new String();
		d += "component if_ne_op_s\n";  
		d += "generic (\n";
		d += "	w_in1	: integer := 16;\n";
		d += "	w_in2	: integer := 16;\n";
		d += "	w_out	: integer := 1\n";
