		...
		for (Signal s : design.getSignals()) {
			...
			if (source.equals(dest)) { // self-connected component 
				...
				if (Parameters.verbose)
					ALP.info("signal " + source + " -> "+ dest + " marked as backedge (self-connected)"); 
			}
			else if (source.isSuccessorOf(dest)) { // self-connected components (pair)
				s.setBackEdge(true);
				if (Parameters.verbose)
					ALP.info("signal " + source + " -> "+ dest + " marked as backedge (self-connected pair)"); 
			}
			else {
				...
				if (sourceLine > 0 && dest instanceof IComponent) {
					...
					if (sourceLine > destLine) { // defined after
						s.setBackEdge(true);
						if (Parameters.verbose)
							ALP.info("signal " + source + " -> "+ dest + " marked as backedge (source defined after destination in source code)"); 
					}
				}
			}
		}
	...
	public int ASAP(Design design) {
		...
		while (change) {
			...
			for (Component dest : design.getComponents()) {
				max = pred = 0;
				if (Parameters.verbose)
					ALP.info("scheduling "+dest+" ASAP:" + dest.getASAP() + " line:" + dest.getLine());
				//all predecessors 
				for (Component source : (Set<Component>)dest.getPredecessors()) {
					...
					if (pred > max) {
						max = pred;
						if (Parameters.verbose)
							ALP.info(" dependence "+source+" ASAP:" + source.getASAP() + " line:" + source.getLine());
					}
				}
				...
				if (components.containsValue(dest)) {
					inner:for (Map.Entry<Integer, Component> me : components.entrySet()) {
						...
						if (line > dest.getLine()) {
							ALP.info(" break on "+source+" ASAP:" + source.getASAP() + " line:" + source.getLine());
							break inner;
						}
						pred = source.getASAP();
						if (pred > max) {
							max = pred;
							if (Parameters.verbose)
								ALP.info(" predecessor "+source+" ASAP:" + source.getASAP() + " line:" + source.getLine());
						}
					}
				}
				if (dest.getASAP() != max) {
	...
	public void balanceAndSyncrhonize(Design design) {
		...
		for (Map.Entry<Integer, Component> me : components.entrySet()) {
			...
			if (c instanceof counter) {
				...
				if (Parameters.verbose)
					ALP.info("using counter " + counter + " to syncronize operations");
				...
				while (s != null) {
					...
					if (dest instanceof block_ram && s.getDestPort().getName().equals("address")) {
						...
						if (distance > 1) {
							...
							ats++;
							if (Parameters.verbose)
								ALP.info("inserting " + (distance-1) + " delay(s) on signal " + counter + " -> "+ dest + " (memory address)");
							String port = "we";
							...
							try {
								ats++;
								if (Parameters.verbose)
									ALP.info("inserting " + (distance-1) + " delay(s) on new signal " + counter + " -> "+ dest + " (memory " + port + ")");
				...
				if (((IComponent)c).getPort("we").getSignal() == null) {
					...
					if (distance > 1) {
						...
						try {
							ats++;
							if (Parameters.verbose)
								ALP.info("inserting " + (distance-1) + " delay(s) on new signal " + counter + " -> "+ c + " (write enable) " + delay);
							counter.connectComponent("step", delay);
					...
					ALP.warning("port 'we' of component " + c + " is connected, please check if aditional synchronization is needed (when ... && "+counter.getName()+".step@N)");
				}
			}
		}
		...
		if (counter != null && done != null && ((IOComponent)done).getPort().getSignal() == null) {
			...
			try {
				ats++;
				if (Parameters.verbose)
					ALP.info("inserting " + amount + " delay(s) on new signal " + counter + " -> "+ done + " (termination)");
		...
		for (Signal signal : signals) {
			...
			if (!(source instanceof delay_op || dest instanceof delay_op || source instanceof mux_m_op || dest instanceof mux_m_op || source instanceof IOComponent || dest instanceof IOComponent || dest.getUserSync())) {
				...
				if (distance > 1) {
					ats++;
					if (Parameters.verbose)
						ALP.info("inserting " + distance + " delay(s) on signal " + source + " -> "+ dest + " (balance)");
		...
		for (Signal signal : signals) {
			if (signal.isBackEdge()) {
				...
				if (source.getSync() && dest.getSync()) {
					if (distance > mii) {
						ALP.warning("backedge " + source + " -> "+ dest + " have distance " + distance + " which is bigger than MII = "+mii+", this can limit MII to lower value");
					}					
				}
				if (Parameters.verbose)
					ALP.info("backedge " + source + " -> "+ dest + " with distance " + distance);
			}
		}
	    t.end();
			...
			for (Component parent : comp.getAllPredecessors()) {
				if (parent instanceof IComponent) {
					...
					if (newMax > max) {
						max = newMax; 
						if (Parameters.verbose)
							ALP.info("calculating ASAP of "+comp+" by "+parent);
					}
				}
			}
			return max;
